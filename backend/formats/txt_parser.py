...
def _extract_chapters(text: str) -> tuple:
    ...
    if matches:
        ...
        for idx, match in enumerate(matches):
            ...
            chapters.append({"title": chapter_title, "position": current_position})
...
def _split_by_size(text: str, words_per_section: int = 2000) -> tuple:
    ...
    for i in range(0, total_words, words_per_section):
        ...
        chapters.append(
            {"title": f"Section {section_num}", "position": current_position}
        )

        content.append(
            {"chapter_index": len(chapters) - 1, "text": _clean_text(section_text)}
        )
