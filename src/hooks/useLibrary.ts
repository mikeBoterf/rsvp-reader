...
export function useLibrary() {
    ...
    const addBook = useCallback((book: Omit<LibraryBook, 'id' | 'currentWordIndex' | 'currentChapter' | 'progressDisplayType' | 'bookmarks' | 'addedAt' | 'lastReadAt'>): LibraryBook => {
    ...
    const updateBookProgress = useCallback((bookId: string, currentWordIndex: number, currentChapter?: number) => {
        setState(prev => ({
            ...prev,
    ...
    const setProgressDisplayType = useCallback((bookId: string, displayType: LibraryBook['progressDisplayType']) => {
        setState(prev => ({
            ...prev,
            books: prev.books.map(book =>
                book.id === bookId ? { ...book, progressDisplayType: displayType } : book
            ),
        }));
    }, []);
    ...
    const activeBook = state.activeBookId ? getBook(state.activeBookId) : undefined;
    ...
    // Bookmark methods
    const addBookmark = useCallback((bookId: string, wordIndex: number, label?: string) => {
        setState(prev => ({
            ...prev,
            books: prev.books.map(book =>
                book.id === bookId
        ...
        setState(prev => ({
            ...
            books: prev.books.map(book =>
                book.id === bookId
                    ? {
                        ...book,
                        bookmarks: book.bookmarks.filter(b => b.wordIndex !== wordIndex)
                    }
                    : book
            ),
        }));
    ...
    const getBookmarks = useCallback((bookId: string): Bookmark[] => {
        const book = state.books.find(b => b.id === bookId);
        return book?.bookmarks || [];
    }, [state.books]);

    const isBookmarked = useCallback((bookId: string, wordIndex: number): boolean => {
