 ...
 * language boundaries.
 */
function getPhraseBounds(words: Word[], startIndex: number, targetSize: number): number {
...
export function useRSVP({
    ...
    const getDelay = useCallback((phraseWords: Word[]) => {
    ...
    const progress = words.length > 0
        ? (phraseStartIndex / words.length) * 100
        : 0;
