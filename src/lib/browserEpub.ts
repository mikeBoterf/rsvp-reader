...
export async function parseBrowserEpub(file: File) {
  ...
  const metadata = await book.loaded.metadata.catch(() => ({} as Record<string, string>));
  const items = (book.spine as unknown as { spineItems: Array<{ idref?: string; render: (load: unknown) => Promise<string>; unload: () => void; }> }).spineItems;
