...
export function RSVPDisplay({
  ...
  if (!phrase || phrase.length === 0) {
    return (
      <div className="rsvp-display rsvp-empty" onClick={onToggle}>
        <div className="rsvp-placeholder">Open an ebook to start reading</div>
      </div>
    );
  }
  ...
  if (phrase.length === 1) {
    ...
    return (
      <div className="rsvp-display" onClick={onToggle}>
        ...
        {!isPlaying && (
          <div className="rsvp-paused-indicator">Press Space to play</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="rsvp-display" onClick={onToggle}>
      <div
        className="rsvp-word-container rsvp-phrase-mode"
      ...
      {phraseSize > 1 && !isPlaying && (
        <div className="rsvp-phrase-indicator">{phraseSize}W mode</div>
      )}
      {!isPlaying && (
        <div className="rsvp-paused-indicator">Press Space to play</div>
      )}
